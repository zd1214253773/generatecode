...
public interface CompilationUnit extends Serializable {
    ...
    default void addImportedTypes(List<String> importedTypes) {
        this.addImportedTypes(importedTypes.stream().map(s->new FullyQualifiedJavaType(s)).collect(Collectors.toSet()));
    }
