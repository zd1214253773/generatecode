...
public class MyBatisGeneratorConfigurationParser {
    ...
    protected void parseTable(Context context, Node node) {
        ...
        String enablelistByPage = attributes
                .getProperty("enablelistByPage"); //$NON-NLS-1$
        if (stringHasValue(enableUpdateByExample)) {
            tc.setListByPageStatementEnabled(
                    isTrue(enablelistByPage));
        }
    ...
    private void parseJavaControllerGenerator(Context context, Node node) {
        ...
        javaControllerGeneratorConfiguration
                .setImplementationPackage(implementationPackage);
