...
public class AnnotatedUpdateByPrimaryKeyWithoutBLOBsMethodGenerator
    ...
    public void addMapperAnnotations(Method method) {
        ...
        for (IntrospectedColumn c : columns) {
            String columnName = escapeStringForJava(getEscapedColumnName(c));
            boolean ignore = false;
            for (String igc : ignorColsWhenUpdate) {
                if (igc.equalsIgnoreCase(columnName)) {
                    ignore = true;
                    break;
                }
            }
            if (!ignore) {
                columnAfterIgnore.add(c);
            }
        }

        iter = columnAfterIgnore.iterator();
